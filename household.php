        ...
        if ($_GET['status'] == 'success') {
          echo '
            <div class="alert alert-success float-end mt-3 mx-2" style="width: 30%;" role="alert">
                ...
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="household.php"></a>
            </div>';
        } elseif ($_GET['status'] == 'error') {
          echo '
            <div class="alert alert-danger float-end mt-3 mx-2" style="width: 30%;" role="alert">
                There was an error adding the household. Please try again.
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="household.php"></a>
            </div>';
        }
      }
...
<?php
      if (isset($_GET['status'])) {
        if ($_GET['status'] == 'update_success') {
          ...
          echo '
            <div class="alert alert-danger float-end mt-3 mx-2" style="width: 30%;" role="alert">
                There was an error updating the household. Please try again.
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="household.php"></a>
            </div>';
        }
      }
...
<?php
      if (isset($_GET['status'])) {
        if ($_GET['status'] == 'delete_success') {
          ...
          echo '
            <div class="alert alert-danger float-end mt-3 mx-2" style="width: 30%;" role="alert">
                There was an error deleting the household. Please try again.
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="household.php"></a>
            </div>';
        }
      }
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0 fs-2">Household</h1>
            </div><!-- /.col -->
          </div><!-- /.row -->
        </div><!-- /.container-fluid -->
      </div>
        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col d-flex justify-content-end">
              <button type="button" class="btn btn-primary mt-5 float-end" data-bs-toggle="modal" data-bs-target="#exampleModal">
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Household Management</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                      </div>
                      <div class="modal-body">
                        <div class="mb-2">
                          <label>HOUSEHOLD NUMBER:</label>
                          <input class="form-control" type="text" placeholder="" name="house_num" required/>			
                        </div>
                        <div class="mb-2">
                          <label>Purok:</label>
                        ...
                        <div class="mb-2">
                          ...
                          <input class="form-control" type="text" placeholder="" name="totalmem" required/>			
                        </div>
                        <div class="mb-2">
                          <label>HEAD OF THE FAMILY:</label>
                          <input class="form-control" type="text" placeholder="" name="head_fam" required/>			
                        </div>
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <button type="submit" name="h_add" class="btn btn-primary">Save</button>
                      </div>
                    </div>
                  </div>   
                </div>
          ...
          <div class="table-responsive mt-4">
            ...
            <table id="datatable" class="table table-bordered border-dark table-striped">
              <thead class="table-primary">
                <tr>
                  <th class="text-center" style="width: 20%;">Household #</th>
                  <th class="text-center" style="width: 20%;">Purok</th>
                  <th class="text-center" style="width: 15%;">Total Members</th>
                  <th class="text-center" style="width: 25%;">Head of Family</th>
                  <th class="text-center" style="width: 20px">Action</th>
                </tr>
              </thead>
              <tbody class="table table-hover">
                <?php
                $sql = $conn->prepare("SELECT * FROM `household` ORDER BY `house_id` DESC");
                $sql->execute();
                while ($row = $sql->fetch()) {
                ?>
                  <tr>
                    <td class="text-center"><?php echo $row['house_num'] ?></td>
                    <td class="text-center"><?php echo $row['purok'] ?></td>
                    <td class="text-center"><?php echo $row['totalmem'] ?></td>
                    <td class="text-center"><?php echo $row['head_fam'] ?></td>
                    <td class="text-center">
                      <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#EhouseModal" data-id="<?php echo $row['house_id']; ?>" data-house_num="<?php echo $row['house_num']; ?>" data-purok="<?php echo $row['purok']; ?>" data-totalmem="<?php echo $row['totalmem']; ?>" data-head_fam="<?php echo $row['head_fam']; ?>">
...
<button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#passcode" data-id="<?php echo $row['house_id']; ?>" <?php echo $disabled; ?>>
...
<script>
    $(document).ready(function () {
       ...
       const newContent = $(data).find('#hr').html();
                    $('#hr').html(newContent);
                    table.destroy();
      ...
      <?php
    				if(ISSET($_GET['house_id'])){
    					...
    					$sql = $conn->prepare("SELECT * FROM `household` WHERE `house_id`='$house_id'");
    					$sql->execute();
    					$row = $sql->fetch();
    				}
    			?>
...
<form method="POST" action="function.php?house_id=<?php echo $house_id?>">
    <div class="modal fade" id="EhouseModal" tabindex="-1" aria-labelledby="EhouseModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="EhouseModalLabel">Household Details</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-2">
                    <input type="hidden" id="edit_house_id" name="house_id">
                        <label>HOUSEHOLD NUMBER:</label>
                        <input class="form-control" type="text" name="house_num" id="edit_house_num" value ="<?php echo $row['house_num']?>" required/>          
                    </div>
                    <div class="mb-2">
                        <label>Purok:</label>
                    ...
                    <div class="mb-2">
                        <label>TOTAL NUMBER OF MEMBERS:</label>
                        <input class="form-control" type="text" name="totalmem" id="edit_totalmem" value ="<?php echo $row['totalmem']?>" required/>          
                    </div>
                    <div class="mb-2">
                        <label>HEAD OF THE FAMILY:</label>
                        <input class="form-control" type="text" name="head_fam" id="edit_head_fam" value ="<?php echo $row['head_fam']?>" required/>          
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="submit" name="h_update" class="btn btn-primary">Save</button>
                </div>
            </div>
        </div>   
    </div>
...
<!-- Password Modal -->
<div class="modal fade" id="passcode" tabindex="-1" aria-labelledby="passcodeLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    ...
    <button type="button" name="ok" class="bn" id="validatePasswordButton">Submit</button> 
...
<div class="modal fade" id="deletehouseModal" tabindex="-1" aria-labelledby="deletehouseModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="deletehouseModalLabel">Delete Confirmation</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
      ...
      <div class="modal-footer">
        ...
        <form method="POST" action="function.php" id="deleteForm">
          ...
          <button type="submit" class="btn btn-primary" name="h_del">Yes</button>
        </form>
      </div>
    </div>
  </div>
...
<script>
 var householdId = document.querySelector('[data-bs-target="#passcode"]').getAttribute('data-id');
var houseIdInput = document.getElementById('house_id');
houseIdInput.value = householdId;

// Add event listener for the submit button inside the passcode modal
var validatePasswordButton = document.getElementById('validatePasswordButton');
validatePasswordButton.addEventListener('click', function () {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            if (response.success) {
                ...
                var passcodeModal = bootstrap.Modal.getInstance(document.getElementById('passcode'));
                passcodeModal.hide(); // Hide the passcode modal

                var deleteModalInstance = new bootstrap.Modal(document.getElementById('deletehouseModal'));
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
        // Handle edit modal
        var editModal = document.getElementById('EhouseModal');
        editModal.addEventListener('show.bs.modal', function (event) {
            var button = event.relatedTarget;
            var houseId = button.getAttribute('data-id');
            var houseNum = button.getAttribute('data-house_num');
            var purok = button.getAttribute('data-purok');
            var totalMem = button.getAttribute('data-totalmem');
            var headFam = button.getAttribute('data-head_fam');

            document.getElementById('edit_house_id').value = houseId;
            document.getElementById('edit_house_num').value = houseNum;
            document.getElementById('edit_purok').value = purok;
            document.getElementById('edit_totalmem').value = totalMem;
            document.getElementById('edit_head_fam').value = headFam;
        });
