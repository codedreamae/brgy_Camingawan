...
<body>
    ...
    <h3 class="text-center"><b>BARANGAY INDIGENCY</b></h3>
<?php
// Fetch only the most recently added data
$sql = $conn->prepare("SELECT * FROM `indigency` ORDER BY `bi_id` DESC LIMIT 1");
$sql->execute();
if ($row = $sql->fetch()) {
?>
    <div class="content">
        ...
        <p style="text-indent: 40px; margin-top: 20px; line-height: 1.5;"> 
           ...
           a resident of Purok <b><?php echo htmlspecialchars($row['purok']); ?></b>, 
            Barangay Camingawan, Kabankalan City, Negros Occidental.
        </p>

        <p style="text-indent: 40px; line-height: 1.5;">
            This certifies futher that this person belongs to an indigent constituent of this
            Barangay. 
        </p>

        <p style="text-indent: 40px; line-height: 1.5;">
            This certification is issued upon the request of the 
            aboved-named person for <b><?php echo htmlspecialchars($row['purpose']); ?></b>
        </p>

        <p style="text-indent: 40px; line-height: 1.5;">
            Given and signed this <b><?php echo date('jS', strtotime($row['issue'])); ?></b> day of <b><?php echo date('F', strtotime($row['issue'])); ?></b>, <b><?php echo date('Y', strtotime($row['issue'])); ?></b> 
