...
<body>
    ...
    <h3 class="text-center"><b>CERTIFICATION</b></h3>
<?php
// Fetch only the most recently added data
$sql = $conn->prepare("SELECT * FROM `certificate` ORDER BY `bc_id` DESC LIMIT 1");
$sql->execute();
if ($row = $sql->fetch()) {
?>
    <div class="content">
        ...
        <p style="text-indent: 40px; margin-top: 20px; line-height: 1.5;"> 
            ...
            a bonafide resident of Purok  <b><?php echo htmlspecialchars($row['purok']); ?></b>, 
            Barangay Camingawan, Kabankalan City, Negros Occidental.
        </p>

        <p style="text-indent: 40px; line-height: 1.5; margin-top: 30px;">
            This certifies futher that  <b><?php echo htmlspecialchars($row['name']); ?></b>, <b><?php echo htmlspecialchars($row['age']); ?></b> years
            old died today/last  <b><?php echo htmlspecialchars($row['death_date']); ?></b> as per certificate of death here to attached.
            The family belongs to an indigent constituent of this Barangay.
        </p>

        <p style="text-indent: 40px; line-height: 1.5; margin-top: 30px;">
            This certification is issued upon the request of his/her  <b><?php echo htmlspecialchars($row['pname']); ?></b>, <b><?php echo htmlspecialchars($row['purpose']); ?></b>, for whatever legal purpose it may serve best.
         </p>

        <p style="text-indent: 40px; line-height: 1.5; margin-top: 30px;">
           <b>GIVEN</b> and <b>SIGNED</b> this <b><?php echo date('jS', strtotime($row['date_complish'])); ?></b> day of <b><?php echo date('F', strtotime($row['date_complish'])); ?></b>, <b><?php echo date('Y', strtotime($row['date_complish'])); ?></b>
           at Barangay Camingawan, Kabankalan City.
        </p>
    </div>
