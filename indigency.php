  ...
  <div class="wrapper">
    ...
    <nav class="main-header navbar navbar-expand navbar-white navbar-light">
      ...
      <ul class="navbar-nav ms-auto">
        <li class="nav-item">
          <a class="nav-link btn btn-primary text-white px-2 mx-2" href="#" role="button">
            <span>Logout</span>
          </a>
        </li>
      </ul>
        ...
        if ($_GET['status'] == 'success') {
          echo '
            <div class="alert alert-success float-end mt-3 mx-2" style="width: 30%;" role="alert">
                indigency added successfully!
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="indigency.php"></a>
            </div>';
        } elseif ($_GET['status'] == 'error') {
          echo '
            <div class="alert alert-danger float-end mt-3 mx-2" style="width: 30%;" role="alert">
                There was an error adding the indigency. Please try again.
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="indigency.php"></a>
            </div>';
        }
      }
...
<?php
      if (isset($_GET['status'])) {
        if ($_GET['status'] == 'update_success') {
          echo '
            <div class="alert alert-success float-end mt-3 mx-2" style="width: 30%;" role="alert">
                indigency updated successfully!
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="indigency.php"></a>
            </div>';
        } elseif ($_GET['status'] == 'update_error') {
          echo '
            <div class="alert alert-danger float-end mt-3 mx-2" style="width: 30%;" role="alert">
                There was an error updating the indigency. Please try again.
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="indigency.php"></a>
            </div>';
        }
      }
...
<?php
      if (isset($_GET['status'])) {
        if ($_GET['status'] == 'delete_success') {
          echo '
            <div class="alert alert-success float-end mt-3 mx-2" style="width: 30%;" role="alert">
                indigency deleted successfully!
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="indigency.php"></a>
            </div>';
        } elseif ($_GET['status'] == 'delete_error') {
          echo '
            <div class="alert alert-danger float-end mt-3 mx-2" style="width: 30%;" role="alert">
                There was an error deleting the indigency. Please try again.
                <a class="text-decoration-none btn-close float-end" aria-label="Close" href="indigency.php"></a>
            </div>';
        }
      }
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0 fs-2">Indigency</h1>
            </div><!-- /.col -->
          </div><!-- /.row -->
        </div><!-- /.container-fluid -->
      </div>
      ...
      <section>
        <div class="container-fluid">
          <div class="row">
            ...
            <button type="button" class="btn btn-primary d-flex align-items-center" style="margin-right: 10px;" data-bs-toggle="modal" data-bs-target="#indiModal">
                <i class="bi bi-plus-square-fill my-2"></i>
                <span>INDIGENCY</span>
              </button>
      <a href="print_indigency.php" class="btn btn-primary d-flex align-items-center" style="margin-right: 10px;">
  <i class="fa fa-print me-2"></i> Print
</a>
              <!-- Modal -->
              <form method="POST" action="function.php">   
                <div class="modal fade" id="indiModal" tabindex="-1" aria-labelledby="indiModalLabel" aria-hidden="true">
                  <div class="modal-dialog">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="indiModalLabel">Indigency Management</h5>
                      ...
                      <div class="mb-2">
                          <input name="issue" class="float-end text-center" value="<?php echo date("Y/m/d");?>">	
                        </div>
                      ...
                      <div class="modal-footer">
                        ...
                        <button type="submit" name="indigency_add" class="btn btn-primary">Save</button>
                      </div>
                    </div>
                  </div>   
                </div>
          ...
          <div class="table-responsive mt-4">
            <table id="datatable" class="table table-bordered border-dark table-striped">
              <thead class="table-primary">
                <tr>
                <th class="text-center" style="width: 15%;">Date of issue</th>
              ...
              <tbody class="table table-hover">
                <?php
                $sql = $conn->prepare("SELECT * FROM `indigency` ORDER BY `bi_id` DESC");
                $sql->execute();
                while ($row = $sql->fetch()) {
                ?>
                  <tr>
                    <td class="text-center"><?php echo $row['issue'] ?></td>
                    <td class="text-center"><?php echo $row['name'] ?></td>
                    <td class="text-center"><?php echo $row['purok'] ?></td>
                    <td class="text-center"><?php echo $row['purpose'] ?></td>
                    <td class="text-center">
                      <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#gencyModal" data-id="<?php echo $row['bi_id']; ?>" data-issue="<?php echo $row['issue']; ?>" data-name="<?php echo $row['name']; ?>" data-purok="<?php echo $row['purok']; ?>" data-purpose="<?php echo $row['purpose']; ?>">
                        <i class="bi bi-pencil-fill"></i>
                        <span>Edit</span>
                      </button>
                      <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteindiModal" data-id="<?php echo $row['bi_id']; ?>">
    ...
    $(document).ready(function () {
       ...
       const newContent = $(data).find('#ireload').html();
                    $('#ireload').html(newContent);
                    table.destroy();
      ...
      <?php
    				if(ISSET($_GET['bi_id'])){
    					$bi_id = $_GET['bi_id'];
    					$sql = $conn->prepare("SELECT * FROM `indigency` WHERE `bi_id`='$bi_id'");
    					$sql->execute();
    					$row = $sql->fetch();
    				}
    			?>
...
<form method="POST" action="function.php?bi_id=<?php echo $bi_id?>">
    <div class="modal fade" id="gencyModal" tabindex="-1" aria-labelledby="gencyModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="gencyModalLabel">Indigency Details</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                <div class="mb-2">
                  <input type="hidden" id="edit_bi_id" name="bi_id">
                 <input class="form-control" type="hidden" name="issue" id="edit_issue" value="<?php echo date("Y/m/d");?>" required/>          
             </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="submit" name="indigency_update" class="btn btn-primary">Save</button>
                </div>
            </div>
        </div>   
    </div>
      ...
      <!-- Delete Modal -->
      <div class="modal fade" id="deleteindiModal" tabindex="-1" aria-labelledby="deleteindiModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h1 class="modal-title fs-5" id="deleteindiModalLabel">Delete Confirmation</h1>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
              ...
              <form method="POST" action="function.php" id="deleteForm">
                ...
                <button type="submit" class="btn btn-primary" name="indigency_del">Yes</button>
              </form>
            </div>
          </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
        // Handle edit modal
        var editModal = document.getElementById('gencyModal');
        editModal.addEventListener('show.bs.modal', function (event) {
            var button = event.relatedTarget;
            var biId = button.getAttribute('data-id');
            var issue = button.getAttribute('data-issue');
            var name = button.getAttribute('data-name');
            var purok = button.getAttribute('data-purok');
            var purpose = button.getAttribute('data-purpose');

            document.getElementById('edit_bi_id').value = biId; 
        ...
        // Handle delete modal
        var deleteModal = document.getElementById('deleteindiModal');
        deleteModal.addEventListener('show.bs.modal', function (event) {
            var button = event.relatedTarget;
            var indigencyId = button.getAttribute('data-id');
            var biIdInput = document.getElementById('bi_id');
            biIdInput.value = indigencyId;
        });
    });
</script>
