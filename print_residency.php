...
<body>
    ...
    <h3 class="text-center"><b>CERTIFICATE OF RESIDENCY</b></h3>
<?php
// Fetch only the most recently added data
$sql = $conn->prepare("SELECT * FROM `residency` ORDER BY `br_id` DESC LIMIT 1");
$sql->execute();
if ($row = $sql->fetch()) {
?>
    <div class="content">
        <p><b>TO WHOM IT MAY CONCERN:</b></p>
        <p style="text-indent: 40px; margin-top: 40px; line-height: 1.5;"> 
            This is to certify that <b><?php echo htmlspecialchars($row['name']); ?></b>,
             <b><?php echo htmlspecialchars($row['age']);?></b> years old, a resident of Purok
             <b><?php echo htmlspecialchars($row['purok']); ?></b>, Barangay Camingawan, Kabankalan City,
             Negros Occidental for the period of  <b><?php echo htmlspecialchars($row['year']); ?></b> years up to present.
            
        </p>

        <p style="text-indent: 40px; line-height: 1.5; margin-top: 30px;">
            This certification is Issued upon the request of the aboved-named person for 
            <b><?php echo htmlspecialchars($row['purpose']); ?></b>.
        </p>

        <p style="text-indent: 40px; line-height: 1.5; margin-top: 30px;">
            GIVEN and SIGNED this <b><?php echo date('jS', strtotime($row['accomplish'])); ?></b> day of <b><?php echo date('F', strtotime($row['accomplish'])); ?></b>, <b><?php echo date('Y', strtotime($row['accomplish'])); ?></b> at Barangay Camingawan,
            Kabakanlan City, Negros Occidental.
        </p>
    </div>
